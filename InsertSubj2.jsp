...
<%
String subjname=request.getParameter("sn");
subjname=subjname.replaceAll("\\s","");
subjname=subjname.toUpperCase();
String facultyname=request.getParameter("fn");
facultyname=facultyname.toUpperCase();
String subjecttype=request.getParameter("st");
String courseyear=request.getParameter("cy");
String coursesem=request.getParameter("cs");
String code="";
code=courseyear+coursesem+subjecttype;
try{
			Class.forName("oracle.jdbc.driver.OracleDriver");
			Connection con=DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:orcl","system","oracle");
            Statement st=con.createStatement();
	       //getting count of subjects for code

		ResultSet rs=st.executeQuery("select code from admin where type='"+subjecttype
			+"'");
		String cde="";
		int count=0;
		int cnt=0;
		...
		int len=cde.length();
		if(cnt!=0)
		count=Integer.parseInt(cde.substring(5,len));
		count++;
			code=code+count;
			//inserting code
			Statement st2=con.createStatement();
			con.setAutoCommit(false);
			st2.executeUpdate("insert into admin (sub,code,fname,type) values ('"+subjname+"','"+code+"','"+facultyname+"','"+subjecttype+"')");
			con.commit();
			Statement st3=con.createStatement();
			con.setAutoCommit(false);
			st3.executeUpdate("create table "+subjname+"(list varchar(20))");
			con.commit();
